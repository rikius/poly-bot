...
async fn main() -> anyhow::Result<()> {
    ...
    {
        let (ledger, obs, cfg, latency, controls, executor, registry, strategy_router) = bot.shared_state();
        let api_state = Arc::new(ApiState::new(ledger, obs, cfg, latency, prometheus_handle, controls, executor, registry, strategy_router));
